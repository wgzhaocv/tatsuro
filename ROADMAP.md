...
- [x] **6.5 播放来源导航 + 下一首播放**(2026-07-16)— 补齐「这首歌从哪来、怎么回去」+ 插队。**下一首播放**:`store.playNext` 插到 `userQueue` 队头(`next()` 先吃队头 = 紧接当前曲播;当前曲 no-op、空队列直接开播),曲目行 ⋯ 菜单入口 + sonner toast(无队列面板,toast 是唯一"加成功"反馈)。**跳转+高亮**:歌单/Liked 行「查看原专辑」、全屏播放器「Open album」(该曲所属专辑)、可点的"Playing from《X》"(跳回队列真正来源:专辑 or 歌单)——全复用分享那套 `?song=` 高亮(4.5s 闪 + 滚动居中),`SharedSongHighlight` 现同挂 album+playlist。队列新增 `contextHref`(随 persist 落盘)存来源 locale-less 路由。新 `lib/player/highlight.ts`(`highlightSong`/`onHighlightSong`/`scrollAndFlash`/`jumpToSong`)解决**站内跳到 `<Activity>` 缓存页不重挂 → 高亮不触发**:冷开走 `?song=` URL、warm 站内跳走 CustomEvent;`scrollAndFlash` 只选**可见**匹配(同 id 可能存在于隐藏缓存页)+ 短暂 rAF 重试(冷跳未访问专辑 250ms 太短)+ cleanup 清 `data-shared`(防二次高亮把上一条 tint 卡死)。**build 坑**:全屏播放器在 `(main)` 静态 shell 里**不能用 next-intl `useRouter`**(prerender 读请求 locale → "uncached data outside Suspense" 炸 build)→ 改 `next/navigation` 原生 router + 手动补 `/${locale}` 前缀。simplify×4 已过、tsc/lint/build 全绿、浏览器实测 play-next toast + 全屏跳转。**队列/历史面板仍未做**(见 #7,站主暂缓)
- [x] **7. 播放队列 / 历史**(2026-07-16 拍板:不做 UI)— **队列引擎阶段 1 已就位**(`usePlayerStore` = context/order/position 状态机 + userQueue/history + shuffle/repeat),此处只指"可拖拽重排的队列/历史**面板**"这一种露出。引擎价值已被三种轻量露出吃掉:迷你条 next / play-next toast(6.5)/ 全屏 transport。可拖拽面板对应的场景(派对多人点歌 / DJ 式手动编排)与**单艺人熟人私享站**最不搭,ROI 低。数据佐证:多数收听是 lean-back(MusicWatch 2021 即便 Premium 也约 25% 纯靠背、Spotify 35% 用在工作/学习/健身背景场景),手动排队列是典型 lean-in / power-user 行为,且各家的可拖拽队列抽屉都埋得很深。**结论:不继续做面板 UI。** 将来真要补,只做"接下来播什么"轻量 peek(不做重排),排在阶段 3 收尾之后。
...
- [x] 数据回归验证(2026-07-16)— 32 releases + 歌词 + MV(13 部)全通,无问题
- [x] 旧站下线或跳转到新站
