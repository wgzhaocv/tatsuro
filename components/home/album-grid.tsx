"use client";

import { useTranslations } from "next-intl";
import { useRef } from "react";
import { BrowseGrid } from "@/components/browse-grid";
import type { Album } from "@/lib/api/types";
import { useFlipReorder } from "@/lib/use-flip-reorder";
import { AlbumCard } from "./album-card";

/** The frosted-glass panel that floats over the hero photo, holding the album
 *  grid. This grid owns the reorder (pins float albums to the front), so it
 *  drives the FLIP animation over its cards (marked with data-flip-id). */
export function AlbumGrid({ albums }: { albums: Album[] }) {
  const t = useTranslations("browse");
  const listRef = useRef<HTMLUListElement>(null);
  useFlipReorder(listRef, albums.map((a) => a.id).join(","));
  return (
    <BrowseGrid
      columns="[grid-template-columns:repeat(auto-fill,minmax(150px,1fr))] md:[grid-template-columns:repeat(auto-fill,minmax(190px,1fr))]"
      isEmpty={albums.length === 0}
      emptyBody={t("albumsEmpty")}
      listRef={listRef}
    >
      {albums.map((album) => (
        <li key={album.id}>
          <AlbumCard album={album} reorderable />
        </li>
      ))}
    </BrowseGrid>
  );
}
